...
export interface SettingsDelegate<ExtArgs extends runtime.Types.Extensions.InternalArgs = runtime.Types.Extensions.DefaultArgs, GlobalOmitOptions = {}> {
   ...
   * 
   * // Only select the `basic`
   * const settingsWithBasicOnly = await prisma.settings.findMany({ select: { basic: true } })
   * 
   */
   ...
   * })
   * 
   * // Create many Settings and only return the `basic`
   * const settingsWithBasicOnly = await prisma.settings.createManyAndReturn({
   *   select: { basic: true },
   *   data: [
   *     // ... provide data here
   *   ]
   ...
   * })
   * 
   * // Update zero or more Settings and only return the `basic`
   * const settingsWithBasicOnly = await prisma.settings.updateManyAndReturn({
   *   select: { basic: true },
   *   where: {
   *     // ... provide filter here
   *   },
