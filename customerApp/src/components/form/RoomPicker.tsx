...
const RoomPicker = ({ rooms, isLoading, value, onChange, seatsMax }: RoomPickerProps) => {
  const [isOpen, setIsOpen] = useState(false);

  const selectedRoom = rooms.find((r) => r.id === value);

  const isOverCapacity =
    !!selectedRoom && seatsMax !== undefined && seatsMax > selectedRoom.capacity;
    ...
    <div className="min-h-22 py-4 w-full bg-background/40 rounded-2xl border border-muted-foreground flex items-center text-foreground cursor-pointer hover:bg-blue-400">
      <Popover open={isOpen} onOpenChange={setIsOpen}>
        <PopoverTrigger asChild>
          <button
            type="button"
            className="w-full h-full pl-9 flex items-center gap-6 cursor-pointer"
          >
              ...
              <div className="w-full flex items-center justify-between">
                ...
                <div className="flex flex-col items-center justify-center mr-7">
                  <ImUsers
                    ...
                    style={{ color: isOverCapacity ? "red" : undefined }}
                  />
                  <span
